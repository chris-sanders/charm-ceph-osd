...
def create_partitions(argv):
    ...
    if args.dmcrypt:
        ...
        # Bluestore
    if args.bluestore:
        setup_data(args, factory)
        populate_list = []
        # Setup block
        if args.block == args.data:
            args.data = 'dummy'
            factory.block.prepare()
            args.data = args.block
        else:
            factory.block.prepare()
        populate_list.append(factory.block)

        # Setup block.db
        if getattr(args, 'block.db'):
            if getattr(args, 'block.db') == args.data:
                args.data = 'dummy'
                factory.blockdb.prepare()
                args.data = getattr(args, 'block.db')
            else:
                factory.blockdb.prepare()
            populate_list.append(factory.blockdb)

        # Setup block.wal
        if getattr(args, 'block.wal'):
            if getattr(args, 'block.wal') == args.data:
                args.data = 'dummy'
                factory.blockwal.prepare()
                args.data = getattr(args, 'block.wal')
            else:
                factory.blockwal.prepare()
            populate_list.append(factory.blockwal)

        # Populate
        factory.data.populate_data_path_device(*populate_list)
    # Filestore
    else:
        # Setup journal
        if args.journal == args.data:
            args.data = 'dummy'
            factory.journal.prepare()
            args.data = args.journal
        else:
            factory.journal.prepare()
        setup_data(args, factory)
        # Populate data
        factory.data.populate_data_path_device(factory.journal)
